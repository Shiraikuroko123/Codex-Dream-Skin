...
- `schemaVersion`：当前可移植主题契约版本为整数 `1`。Windows 继续读取缺少该字段的旧主题并在下次写入时迁移为版本 1；显式的未知版本会拒绝，不会按旧语义静默加载。
...
- Windows 导入会拒绝空文件、超过 12 MB、任一边超过 4096px 或总像素超过 12MP 的输入；macOS 快速加载入口仍使用 16 MB、16384px 与 50MP 上限，主定制入口可接收最高 50 MB 的源图，但转换后的主题文件必须不超过 16 MB。Windows 在复制导入图前复用 Node 元数据解析器执行尺寸限制，注入器再用图片与主题内容的 SHA-256 修订值识别热更新，并在构建首帧 payload 前同步读取图片比例。
